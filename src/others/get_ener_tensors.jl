...
function get_ener_tensors(physical_legs,J1,J2,J3,dbetasu)
    ...
    h = double_lattice("tensors")
    ener = double_lattice("tensors")
    hex = double_lattice("tensors")
    singlehex = double_lattice("tensors")
    ...
    for i = 1:3
        ...
        # AB
        hy =  hy + 
        J1*kron(id,S[i],id,S[i],id,id) + # 24
        J1*kron(id,id,S[i],id,id,S[i]) + # 36
        J3*kron(id,S[i],S[i],id,id,id) + # 23
        J3*kron(id,id,id,S[i],id,S[i]) + # 46
        J2*kron(id,S[i],id,id,id,S[i])   # 26

        # BC
        hx = hx + 
        J1*kron(id,S[i],id,id,S[i],id) + # 25
        J1*kron(id,id,S[i],S[i],id,id) + # 34
        J3*kron(id,S[i],S[i],id,id,id) + # 23
        J3*kron(id,id,id,S[i],S[i],id) + # 45
        J2*kron(id,S[i],id,S[i],id,id)   # 24
    
        # CD
        hz = hz + 
        J1*kron(S[i],id,id,S[i],id,id) + # 14
        J1*kron(id,id,S[i],id,S[i],id) + # 35
        J3*kron(S[i],id,S[i],id,id,id) + # 13
        J3*kron(id,id,id,S[i],S[i],id) + # 45
        J2*kron(id,id,S[i],S[i],id,id)   # 34
    ...
    names = fieldnames(double_lattice)
    for i = 1:9
        setproperty!(hex1array, names[i], reshape(getproperty(hex1array,names[i]), (8,8,8,8)))
        setproperty!(hex2array, names[i], reshape(getproperty(hex2array,names[i]), (8,8,8,8)))
    end


    charges = [-3, -1, -1, 1, -1, 1, 1, 3]
    # charges = [0,1,1,2,1,2,2,3]
